...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="Custom output file path for JSON results (optional). Default: saves to output/ folder"
    )
    parser.add_argument(
        "--no-sql",
        action="store_true",
        help="Skip saving SQL files (only save JSON results)"
    )
    ...
    if len(args.files) == 1:
        result = process_single_file(args.files[0], args.batch_size, results_dir=results_dir)
    else:
        result = process_multiple_files(args.files, args.batch_size, results_dir=results_dir)
    ...
    if args.no_sql:
        ...
        save_results(result, output_dir, args.output, results_dir)
